defmodule ExQuickBooks.Endpoint.JSONTest do
  ...
  test "send_json_request/1 parses JSON responses" do
    ...
    assert {:ok, %{"foo" => true}} = request(:get, "path") |> send_json_request
  end
  ...
  test "send_json_request/1 parses JSON error responses" do
    ...
    assert {:error, %{"foo" => true}} = request(:get, "path") |> send_json_request
  end
  ...
  test "send_json_request/1 doesn't parse non-JSON responses" do
    ...
    assert {:ok, %HTTPoison.Response{body: "foo"}} = request(:get, "path") |> send_json_request
  end
end
